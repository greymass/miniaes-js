...
suite('aes', function () {
    test('cbc', function () {
        ...
        for (let i = 0; i < cbc_aes_vectors.length; ++i) {
            ...
            assert.equal(
                bytes_to_hex(lib.AES_CBC.encrypt(clear, key, false, iv)),
                bytes_to_hex(cipher),
                `encrypt vector ${i}`
            )

            assert.equal(
                bytes_to_hex(lib.AES_CBC.decrypt(cipher, key, false, iv)),
                bytes_to_hex(clear),
                `decrypt vector ${i}`
            )
        }
...
function bytes_to_hex(arr: Uint8Array): string {
    let str = ''
    for (let i = 0; i < arr.length; i++) {
        const h = (arr[i] & 0xff).toString(16)
        if (h.length < 2) str += '0'
        str += h
    }
    return str
